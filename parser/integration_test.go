...
func setupTestDatabases(t *testing.T) map[DataType]testDB {
	dbs := make(map[DataType]testDB)
	...
	if pgURL := os.Getenv("TEST_POSTGRES_URL"); pgURL != "" {
		...
		dbs[TypePostgreSQL] = testDB{
			db:      db,
			driver:  "postgres",
			version: &Version{Major: 12},
			cleanup: func() {
				db.Close()
			},
		}
...
func TestIntegration_DataTypeConversion(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			sourceType: TypeMySQL,
			targetType: TypePostgreSQL,
