...
import (
	...
	"github.com/mstgnz/sqlporter/parser/mysql"
	"github.com/mstgnz/sqlporter/parser/oracle"
	"github.com/mstgnz/sqlporter/parser/postgres"
	"github.com/mstgnz/sqlporter/parser/sqlite"
	"github.com/mstgnz/sqlporter/parser/sqlserver"
)
...
func BenchmarkSQLServerParser_Parse(b *testing.B) {
	parser := &sqlserver.SQLServerParser{}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := parser.Parse(sampleSQL)
		if err != nil {
...
func BenchmarkMySQLToPostgres_Convert(b *testing.B) {
	...
	pgParser := &postgres.PostgresParser{}
...
func BenchmarkMySQLToSQLite_Convert(b *testing.B) {
	mysqlParser := &mysql.MySQLParser{}
	sqliteParser := &sqlite.SQLiteParser{}
...
func BenchmarkConverter_ConvertType(b *testing.B) {
	...
	sourceVer := &parser.Version{Major: 5, Minor: 7}
	targetVer := &parser.Version{Major: 12}
...
func BenchmarkStreamParser_Parse(b *testing.B) {
	parser := parser.NewStreamParser(parser.StreamParserConfig{
		Workers:    4,
		BatchSize:  1024 * 1024,
		BufferSize: 32 * 1024,
	})
	...
	for i := 0; i < b.N; i++ {
		_, err := parser.ParseStream(context.Background(), strings.NewReader(sql))
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkBatchProcessor_Process(b *testing.B) {
	processor := parser.NewBatchProcessor(parser.BatchConfig{
		BatchSize: 1000,
		Workers:   4,
	})

	// Generate test statements
	stmts := make([]*parser.Statement, 1000)
	for i := range stmts {
		stmts[i] = parser.NewStatement(
	...
	for i := 0; i < b.N; i++ {
		err := processor.ProcessBatch(context.Background(), stmts, func(stmt *parser.Statement) error {
			return nil
		})
		if err != nil {
			b.Fatal(err)
